...
// ================= API =================
const API = "https://script.google.com/macros/s/AKfycbytGuQBA7tq5-YkRE5LGNkS-CyRTTygpIyiYvdn7sVcx3lYxwdGjUMfxo6S077AtWtO/exec";
...
async function apiPost(body) {
  ...
  try {
    ...
    if (location.protocol === "file:") {
      alert("⚠️ Run using Live Server (not file://)");
    }
...
async function loadBusinesses() {
  ...
  if (list) {
    ...
    formatted.forEach(b => {
      ...
      const isActive = b.id == currentBusiness;
      ...
      listHtml += `
        <div class="bg-gray-800 p-3 rounded-xl flex justify-between items-center
          ...
          <!-- LEFT -->
          <div onclick="selectBusiness('${b.id}')"
...
function selectBusiness(id) {
  ...
  const selected = window.businesses.find(b => b.id == id);

  businessName = selected ? selected.name : "My Business";

  businessSelect.value = id;

  loadBusinesses(); // refresh UI highlight
  openCustomers();
  checkWhatsAppStatus();
}
...
async function sendTxnWhatsApp(
  ...
  try {
    ...
    console.log("TXN WA RESPONSE:", res);
...
function detectTxnMeta(note = "") {
  ...
  // =====================================
  // 1️⃣ CHECK CUSTOM RULES
  // =====================================
  for (let rule of customRules) {
    if (rule.words.some(w => note.includes(w))) {
      return rule;
    }
  }

  // =====================================
  // 2️⃣ CHECK GOOGLE SHEET RULES
  // =====================================
  ...
  for (let rule of sheetRules) {
    ...
    if (rule.words.some(w =>
      note.includes(String(w).toLowerCase())
    )) {
      return rule;
    }
  }
...
function getSheetTxnRules() {
  ...
  // Already loaded
  if (sheetTxnRulesLoaded) {
    return sheetTxnRulesCache;
  }

  // Prevent multiple calls
  if (!sheetTxnRulesLoading) {
    ...
    apiGet({
      ...
      .then(rows => {
        ...
        if (Array.isArray(rows)) {
          ...
          console.log("✅ Rules Loaded:", rows.length);
        }

        sheetTxnRulesLoaded = true;
        sheetTxnRulesLoading = false;

      })
      .catch(err => {
        console.log("❌ Rule Load Error", err);
        sheetTxnRulesLoaded = true;
        sheetTxnRulesLoading = false;
      });
  }
