...
async function saveAccount(btn) {
  ...
  if (!name) {
    msg.className = "text-red-400 text-sm mb-2";
    msg.innerText = "Enter account name ❌";
    msg.classList.remove("hidden");
    return;
  }

  // ✅ duplicate check
  const exists = (window.accountsData || []).some(
  ...
  try {
    ...
    // ✅ SUCCESS MESSAGE
    msg.className = "text-green-400 text-sm mb-2";
    msg.innerText = "Account added ✅";
    msg.classList.remove("hidden");
