...
async function markAsPaid(card) {
  ...
  confirmBox.className = "absolute inset-0 bg-black/80 flex items-center justify-center z-50";
  confirmBox.innerHTML = `
    <div class="bg-gray-900 p-4 rounded text-center">
      <div class="mb-3">Mark as paid?</div>
      <div class="flex gap-2">
        <button class="bg-gray-700 px-3 py-1 rounded">Cancel</button>
        <button class="bg-green-600 px-3 py-1 rounded">Yes</button>
      </div>
    </div>
  `;
  ...
  const [cancelBtn, yesBtn] = confirmBox.querySelectorAll("button");
