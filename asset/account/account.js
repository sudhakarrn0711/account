...
// ================= API =================
const API = "https://script.google.com/macros/s/AKfycbwDUxOlkYoWpQs2BGFRhGNrb9XwEIsLcbVE-2xTF2kB0LC8fh6HPgQDyJhPOeS1W71f/exec";
...
async function apiGet(action, params = {}) {
  ...
  let url = `${API}?action=${action}&env=${env}`;
...
function downloadSampleExcel() {
  ...
  const sampleData = [
    ...
    { Name: "Ravi Kumar", Phone: "9123456789" }
  ];
  ...
  XLSX.writeFile(workbook, "Customer_Bulk_Upload_Sample.xlsx");
}
...
async function sendWhatsAppReminder(name, phone, amount) {
  ...
  try {
    ...
    let summaryLink = `https://accounts.ransangroups.in/account_dashboard.html?customer=${selectedCustomer}&bid=${currentBusiness}`;
...
async function sendWhatsAppInviteAPI(name, phone) {
